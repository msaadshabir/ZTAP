...
- **Logging: `internal/logging` reimplemented over `log/slog`** (Phase F of the modernization plan). The public API and the emitted JSON/text schema are unchanged — golden tests in `internal/logging/testdata/` pin the exact `{"timestamp","level","message","fields"}` lines. Internals are now a thin shim over `slog.Logger` with a custom handler preserving the historical schema (UTC RFC3339 timestamps, lowercase levels, sanitized messages, sorted field keys).
- **Logging: global logger state removed**. The package-level lazy singleton (`logging.Default()`) and its `exitFn` test seam are gone. Package helpers delegate to `slog.Default()`; `New`/`Configure` install the logger as the slog default, which also bridges stdlib `log` output (etcd, client-go, ...) at info level — replacing the previous hand-rolled `log.SetOutput` hijack. Before the first `Configure` call (CLI always configures in `PersistentPreRunE`), helpers fall back to the stdlib default (text to stderr) instead of a lazily-created JSON logger on stdout.
- **Operator: unified structured logging**. `ztap-operator` replaces its zap development-mode logger with the same `log/slog` core as the main binary via `logr.FromSlogHandler`; it honors `ZTAP_LOG_LEVEL`/`ZTAP_LOG_FORMAT`/`ZTAP_LOG_FILE` (or the shared defaults) and emits the same JSON/text schema. The `--zap-*` flag family is removed.
- **Logging hardening**: concurrent slog records are serialized, `LogValuer` and error fields retain their values in JSON, logr verbosity levels are normalized to ZTAP's `debug`/`info`/`warn`/`error` schema, and operator logging configuration failures are reported to stderr before logger installation.
