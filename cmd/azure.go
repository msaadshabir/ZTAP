...
var azureNSGSyncCmd = &cobra.Command{
...
func init() {
	azureNSGSyncCmd.Flags().String("subscription-id", "", "Azure subscription ID")
	azureNSGSyncCmd.Flags().String("resource-group", "", "Azure resource group containing the NSG")
	azureNSGSyncCmd.Flags().String("nsg", "", "Target Azure NSG name")
	azureNSGSyncCmd.Flags().String("rule-prefix", "", "Rule name prefix (optional, defaults to ztap-)")
	azureNSGSyncCmd.Flags().Int32("priority-base", 0, "Base priority for managed rules (optional)")

	azureCmd.AddCommand(azureNSGSyncCmd)
