...
Note: selector-based rules (`podSelector`, optionally with `namespaceSelector`) require resolution into concrete IPs.
In Kubernetes, targets are translated into host CIDRs (`/32` for IPv4, `/128` for IPv6) via discovery (recommended: `ztap agent`), or via `ztap enforce` when `discovery.backend: k8s` is configured.
For GCP firewall sync, `podSelector` is resolved against GCE instance labels within the specified VPC network.
