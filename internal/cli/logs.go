...
func newLogsCmd() *cobra.Command {
	...
	c.Flags().StringP("policy", "p", "", "Filter by policy name (substring match)")
	c.Flags().StringP("level", "l", "", "Minimum level to show (debug, info, warn, error)")
	c.Flags().StringP("contains", "c", "", "Filter messages containing substring")
	c.Flags().BoolP("follow", "f", false, "Follow log output")
	c.Flags().IntP("tail", "n", 0, "Show last N entries (0 = all)")
	return c
}
