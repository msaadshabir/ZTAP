...
func newAwsInventoryCmd() *cobra.Command {
	c := &cobra.Command{
		Use:   "inventory",
	...
	c.AddCommand(newAwsInventoryExportCmd())
	c.AddCommand(newAwsInventoryResolveCmd())
	return c
}

func newAwsInventoryExportCmd() *cobra.Command {
	c := &cobra.Command{
		Use:   "export",
	...
	c.Flags().String("region", "us-east-1", "AWS region")
	c.Flags().String("profile", "", "AWS CLI profile")
	c.Flags().String("out", "", "Output file (stdout if empty or '-')")
	return c
}

func newAwsInventoryResolveCmd() *cobra.Command {
	c := &cobra.Command{
		Use:   "resolve",
	...
	c.Flags().StringToString("labels", nil, "Label selectors (key=value)")
	c.Flags().String("selector", "", "K8s-style selector string (e.g., 'app in (web,api),tier!=dev')")
	c.Flags().String("inventory-file", "", "Path to pre-exported inventory JSON")
	c.Flags().String("ip-mode", "private", "Which IPs to resolve (private, public, both)")
	c.Flags().String("region", "us-east-1", "AWS region (used when --inventory-file not provided)")
	c.Flags().String("profile", "", "AWS CLI profile (used when --inventory-file not provided)")
	return c
}
