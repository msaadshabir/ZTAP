...
func newPolicyCmd() *cobra.Command {
	...
	// Add subcommands
	c.AddCommand(newPolicySyncCmd())
	c.AddCommand(newPolicyListCmd())
	c.AddCommand(newPolicyWatchCmd())
...
func newPolicySyncCmd() *cobra.Command {
	c := &cobra.Command{
...
Only the leader can initiate policy synchronization.`,
	...
	// Add flags
	c.Flags().StringP("name", "n", "", "Policy name (defaults to filename)")
	return c
}
...
func newPolicyHistoryCmd() *cobra.Command {
	...
	c.Flags().Int("limit", 10, "Number of revisions to display (0 for all)")
	return c
}
...
func newPolicyRollbackCmd() *cobra.Command {
	...
	c.Flags().Int64("to", 0, "Target version to rollback to")
	c.Flags().String("reason", "", "Optional reason for rollback")
	return c
}
