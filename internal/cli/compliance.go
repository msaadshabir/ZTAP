...
func newComplianceCmd() *cobra.Command {
	...
	c.AddCommand(newComplianceExportCmd())
...
func newComplianceExportCmd() *cobra.Command {
	...
	c.Flags().StringArrayP("policy-file", "f", nil, "Path to policy YAML file (repeatable)")
	c.Flags().Bool("policy-yaml-stdin", false, "Read additional policy YAML from stdin")
	c.Flags().String("policy-name", "", "Policy key for audit correlation (matches audit resource; accepts tenant/name)")
	c.Flags().String("mapping-file", "", "Path to compliance mapping YAML")
	c.Flags().StringArray("framework", nil, "Framework IDs to include (pci-dss,soc2,hipaa)")
	c.Flags().String("audit-log", "", "Path to audit log (defaults to ~/.ztap/audit.log if present)")
	c.Flags().String("evidence-window", "90d", "Lookback window for enforcement evidence (e.g. 90d, 2160h, 30m)")
	c.Flags().Bool("strict", false, "Fail on unknown frameworks or invalid control IDs")
	c.Flags().String("format", "json", "Output format: json|csv")
	c.Flags().String("out", "-", "Output path ('-' for stdout)")
	return c
}
...
func newComplianceReportCmd() *cobra.Command {
	...
	c.Flags().String("evidence-window", "90d", "Lookback window for enforcement evidence (e.g. 90d, 2160h, 30m)")
	c.Flags().Bool("strict", false, "Fail on unknown frameworks or invalid control IDs")
	c.Flags().String("format", "md", "Output format: md|json")
	c.Flags().String("out", "-", "Output path ('-' for stdout)")
	c.Flags().String("out-dir", "", "Write bundle to a directory (report.md + report.json)")
	return c
}
