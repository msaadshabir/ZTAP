...
func TestBuildReportFromAnnotationsWithEvidence(t *testing.T) {
	...
	report, err := BuildReport(nil, []policy.NetworkPolicy{p}, BuildOptions{
...
func TestAuditIntegrityFailureIsReported(t *testing.T) {
	...
	report, err := BuildReport(nil, []policy.NetworkPolicy{p}, BuildOptions{AuditLogPath: auditPath, EvidenceWindow: 24 * time.Hour})
	if err != nil {
		t.Fatalf("BuildReport: %v", err)
	}
...
`)
	...
	report, err := BuildReport(nil, []policy.NetworkPolicy{p}, BuildOptions{MappingFileYAML: mappingYAML})
	if err != nil {
		t.Fatalf("BuildReport: %v", err)
	}
...
func TestEnforcementEvidenceRespectsWindow(t *testing.T) {
	...
	report, err := BuildReport(nil, []policy.NetworkPolicy{p}, BuildOptions{AuditLogPath: auditPath, EvidenceWindow: 1 * time.Millisecond})
	if err != nil {
		t.Fatalf("BuildReport: %v", err)
	}
	if len(report.Policies) != 1 {
