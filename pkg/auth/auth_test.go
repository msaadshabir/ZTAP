...
func TestAuthenticate(t *testing.T) {
	...
	session, err := manager.Authenticate(t.Context(), "testuser", "correctpassword")
	if err != nil {
	...
	_, err = manager.Authenticate(t.Context(), "testuser", "wrongpassword")
	if err == nil {
		t.Error("Expected error for wrong password")
	}

	_, err = manager.Authenticate(t.Context(), "nonexistent", "password")
	if err == nil {
		t.Error("Expected error for nonexistent user")
	}

	_ = manager.DisableUser("testuser")
	_, err = manager.Authenticate(t.Context(), "testuser", "correctpassword")
	if err == nil {
...
func TestValidateSession(t *testing.T) {
	...
	session, _ := manager.Authenticate(t.Context(), "testuser", "password")
	...
	if err := manager.store.Put(t.Context(), session.Token, session); err != nil {
...
func TestHasPermission(t *testing.T) {
	...
	adminSession, _ := manager.Authenticate(t.Context(), "admin2", "pass")
	operatorSession, _ := manager.Authenticate(t.Context(), "operator", "pass")
	viewerSession, _ := manager.Authenticate(t.Context(), "viewer", "pass")
...
func TestChangePassword(t *testing.T) {
	...
	_, err = manager.Authenticate(t.Context(), "testuser", "oldpassword")
	if err == nil {
		t.Error("Old password still works")
	}

	_, err = manager.Authenticate(t.Context(), "testuser", "newpassword")
	if err != nil {
...
func TestDefaultAdmin(t *testing.T) {
	...
	_, err := manager.Authenticate(t.Context(), "admin", "ztap-admin-change-me")
	if err != nil {
